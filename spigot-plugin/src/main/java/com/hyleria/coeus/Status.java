...
public enum Status
{
    ...
    LOBBY_COUNTDOWN,     // the required players have joined us; we're starting the countdown [may back-out still]
    NO_RETURN,     // we've hit 10 seconds left of the countdown; the map is being loaded now (unless it's handled specially)
    PRE_GAME,      // [actual match created] initial game logic is loading & in-world countdown (if enabled)
